...
def generate_submission(input_path: Path, output_root: Path = None, top_k: int = 100):
    ...
    # Khởi tạo TaskSpecializedEngine (Chuyên biệt hóa KIS, QA, TRAKE)
    engine = TaskSpecializedEngine(engine="siglip2")
    ...
    for idx, q_info in enumerate(queries, 1):
        ...
        print(f"    ✅ Đã ghi {len(lines)} dòng vào: outputs/submission/{csv_filename}")
...
def main():
    ...
    parser.add_argument("--input", type=str, default="data/benchmark/ground_truth.json", help="Thư mục chứa file .txt hoặc đường dẫn file .json")
    parser.add_argument("--output_dir", type=str, default="outputs", help="Thư mục lưu submission và file zip (mặc định 'outputs')")
    ...
    generate_submission(input_path=in_path, output_root=out_root, top_k=args.top_k)
